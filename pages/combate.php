...
if (!$id_registro || !$id_partida) die("Error: Sesión o enemigo no encontrado.");
...
if ($vida_jugador === false || $vida_jugador === null) $vida_jugador = 100;

if (!$enemigo) { header("Location: juego.php?sala=" . $sala_vuelta); exit; }
...
<head>
    ...
    <style>
        ...
        #vats-container { width: 100vw; height: 100vh; display: flex; flex-direction: column; background: radial-gradient(circle, #001100 0%, #000 100%); }
        .main-view { flex: 1; position: relative; display: flex; justify-content: center; align-items: center; }
        .enemy-sprite { height: 70vh; filter: sepia(1) brightness(0.8) hue-rotate(80deg) drop-shadow(0 0 10px var(--vats-green)); }
        .vats-node { position: absolute; background: var(--vats-bg); border: 1px solid var(--vats-green); padding: 10px; cursor: pointer; transition: 0.2s; z-index: 100; min-width: 80px; text-align: center; }
        .vats-node:hover { background: var(--vats-green); color: #000; font-weight: bold; }
        .node-head { top: 15%; left: 55%; }
        .node-torso { top: 40%; left: 60%; }
        .node-legs { top: 70%; left: 55%; }
        .vats-footer { height: 160px; border-top: 2px solid var(--vats-green); display: grid; grid-template-columns: 1.5fr 1.5fr 1fr; background: rgba(0, 10, 0, 0.95); padding: 15px; gap: 20px; }
        .stat-box { border: 1px solid var(--vats-green); padding: 10px; display: flex; flex-direction: column; justify-content: center; }
        ...
        .ammo-empty { color: #ff4444; }
        
        .weapon-selector { display: flex; gap: 10px; margin-top: 5px; }
        .weapon-btn { border: 1px solid var(--vats-green); background: rgba(0,40,0,0.8); color: var(--vats-green); padding: 5px; cursor: pointer; font-size: 0.6rem; transition: 0.3s; flex: 1; text-align: center; }
        .weapon-btn.active { background: var(--vats-green); color: #000; font-weight: bold; }
        .weapon-btn:hover { background: rgba(0,255,102,0.2); }
    </style>
</head>
<body>
...
<div id="vats-container">
    ...
    <div class="main-view">
        ...
        <div class="vats-node node-head" onclick="procesarAccion('cabeza', <?php echo $enemigo['precision_cabeza']; ?>)">
            <span style="font-size: 1.2rem;"><?php echo $enemigo['precision_cabeza']; ?>%</span><br><small>CABEZA</small>
        </div>
        <div class="vats-node node-torso" onclick="procesarAccion('torso', <?php echo $enemigo['precision_torso']; ?>)">
            <span style="font-size: 1.2rem;"><?php echo $enemigo['precision_torso']; ?>%</span><br><small>TORSO</small>
        </div>
        <div class="vats-node node-legs" onclick="procesarAccion('piernas', <?php echo $enemigo['precision_piernas']; ?>)">
            <span style="font-size: 1.2rem;"><?php echo $enemigo['precision_piernas']; ?>%</span><br><small>PIERNAS</small>
        </div>
    </div>
    ...
    <div class="vats-footer">
        <div class="stat-box" style="border-color: #ffb000; color: #ffb000;">
            ...
            <div style="font-size: 1.2rem; font-weight: bold;">HP: <span id="hp-p-text"><?php echo $vida_jugador; ?></span> / 100</div>
            <div class="hp-bar-bg" style="border-color: #ffb000;">
                <div id="hp-p-bar" class="hp-bar-fill hp-player-fill" style="width: <?php echo $vida_jugador; ?>%"></div>
            </div>
            ...
            <div class="log-box" id="combat-log" style="margin-top: 5px; color: #ffb000;">> ESPERANDO ÓRDENES...</div>
        </div>

        <div class="stat-box">
            <div>OBJETIVO (ESTADO)</div>
            <div style="font-size: 1.2rem; font-weight: bold;">HP: <span id="hp-e-text"><?php echo $enemigo['vida_restante']; ?></span></div>
            <div class="hp-bar-bg">
                <div id="hp-e-bar" class="hp-bar-fill" style="width: <?php echo ($enemigo['vida_restante']/$enemigo['vida_maxima'])*100; ?>%"></div>
            </div>
        </div>
