...
function renderizarInventario(items) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    slot.addEventListener("drop", (e) => {
      ...
      if (
        draggedItemInfo &&
        draggedItemInfo.sourceSlotIndex !== targetSlotIndex
      ) {
        const targetItem = itemMap[targetSlotIndex];

        if (targetItem) {
          combinarObjetos(
            draggedItemInfo.idRegistro,
            targetItem.id_registro,
            targetSlotIndex,
          );
        } else {
    ...
    if (itemMap[i]) {
      ...
      itemElement.addEventListener("click", () => {
        document
          ...
          .forEach((s) => (s.style.borderColor = "#333"));
        ...
        if (btnPuzzle) {
          const esCajaPortatil =
            item.nombre &&
            item.nombre.toUpperCase().includes("CAJA FUERTE PORTATIL");
          if (esCajaPortatil) {
        ...
        if (btnEliminar) {
          if (item.tipo !== "arma" && item.tipo !== "clave") {
            ...
            btnEliminar.onclick = () => {
              if (
                confirm(`¿Estás seguro de que quieres eliminar ${item.nombre}?`)
              ) {
